@extends('admin.layouts.master')

@section('page-title')
    تصحيح اختبارات الكورسات
@stop

@section('content')
    <div class="main-content app-content">
        <div class="container-fluid">

            <!-- Alerts -->
            @include('admin.partials.ui.alerts')

            <!-- Page Header -->
            <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
                <div class="my-auto">
                    <h5 class="page-title fs-21 mb-1">تصحيح اختبارات الكورسات</h5>
                    <nav>
                        <ol class="breadcrumb mb-0">
                            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">لوحة التحكم</a></li>
                            <li class="breadcrumb-item active">تصحيح اختبارات الكورسات</li>
                        </ol>
                    </nav>
                </div>
            </div>

            <!-- Statistics Cards -->
            @php
                $pendingCount = 0;
                $partiallyGradedCount = 0;
                $fullyGradedCount = 0;
                foreach ($attempts as $attempt) {
                    if ($attempt->pending_grading_count > 0) {
                        $pendingCount++;
                    } elseif ($attempt->pending_grading_count == 0 && $attempt->responses->count() > 0) {
                        $fullyGradedCount++;
                    }
                }
            @endphp

            <div class="row mb-4">
                <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12">
                    <div class="card custom-card">
                        <div class="card-body">
                            <div class="d-flex align-items-start justify-content-between">
                                <div>
                                    <p class="mb-1 text-muted">بانتظار التصحيح</p>
                                    <h3 class="mb-0 fw-semibold text-danger">{{ $pendingCount }}</h3>
                                </div>
                                <div>
                                    <span class="avatar avatar-md bg-danger-transparent">
                                        <i class="fas fa-exclamation-circle fs-18"></i>
                                    </span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12">
                    <div class="card custom-card">
                        <div class="card-body">
                            <div class="d-flex align-items-start justify-content-between">
                                <div>
                                    <p class="mb-1 text-muted">مُصحح بالكامل</p>
                                    <h3 class="mb-0 fw-semibold text-success">{{ $fullyGradedCount }}</h3>
                                </div>
                                <div>
                                    <span class="avatar avatar-md bg-success-transparent">
                                        <i class="fas fa-check-circle fs-18"></i>
                                    </span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12">
                    <div class="card custom-card">
                        <div class="card-body">
                            <div class="d-flex align-items-start justify-content-between">
                                <div>
                                    <p class="mb-1 text-muted">إجمالي المحاولات</p>
                                    <h3 class="mb-0 fw-semibold">{{ $attempts->total() }}</h3>
                                </div>
                                <div>
                                    <span class="avatar avatar-md bg-primary-transparent">
                                        <i class="fas fa-file-alt fs-18"></i>
                                    </span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-xl-3 col-lg-6 col-md-6 col-sm-12">
                    <div class="card custom-card">
                        <div class="card-body">
                            <div class="d-flex align-items-start justify-content-between">
                                <div>
                                    <p class="mb-1 text-muted">إجمالي الأسئلة</p>
                                    <h3 class="mb-0 fw-semibold">
                                        {{ $attempts->sum(function($attempt) { return $attempt->responses->count(); }) }}
                                    </h3>
                                </div>
                                <div>
                                    <span class="avatar avatar-md bg-info-transparent">
                                        <i class="fas fa-question-circle fs-18"></i>
                                    </span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Attempts Table -->
            <div class="card custom-card">
                <div class="card-header">
                    <div class="card-title">
                        قائمة المحاولات المُسلمة ({{ $attempts->total() }})
                    </div>
                </div>
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-hover text-nowrap">
                            <thead>
                                <tr>
                                    <th width="5%">#</th>
                                    <th width="20%">الطالب</th>
                                    <th width="20%">الاختبار</th>
                                    <th width="10%">المحاولة</th>
                                    <th width="12%">تاريخ الإنهاء</th>
                                    <th width="10%">الدرجة</th>
                                    <th width="13%">حالة التصحيح</th>
                                    <th width="10%">الإجراءات</th>
                                </tr>
                            </thead>
                            <tbody>
                                @forelse($attempts as $attempt)
                                    <tr class="{{ $attempt->pending_grading_count > 0 ? 'bg-danger-transparent' : '' }}">
                                        <td>{{ $loop->iteration + ($attempts->currentPage() - 1) * $attempts->perPage() }}</td>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                <div class="avatar avatar-sm bg-primary-transparent me-2">
                                                    <i class="fas fa-user"></i>
                                                </div>
                                                <div>
                                                    <span class="fw-semibold">{{ $attempt->student->name ?? 'غير معروف' }}</span>
                                                    <br>
                                                    <small class="text-muted">{{ $attempt->student->email ?? '-' }}</small>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <span class="fw-semibold">{{ $attempt->questionModule->title ?? 'غير معروف' }}</span>
                                        </td>
                                        <td>
                                            <span class="badge bg-secondary-transparent">#{{ $attempt->attempt_number }}</span>
                                        </td>
                                        <td>
                                            <small>{{ $attempt->completed_at ? $attempt->completed_at->format('Y-m-d H:i') : '-' }}</small>
                                        </td>
                                        <td>
                                            @if($attempt->total_score !== null)
                                                <span class="badge bg-{{ $attempt->is_passed ? 'success' : 'danger' }}">
                                                    {{ number_format($attempt->percentage ?? 0, 1) }}%
                                                </span>
                                                <br>
                                                <small class="text-muted">{{ number_format($attempt->total_score, 1) }}</small>
                                            @else
                                                <span class="text-muted">-</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if($attempt->pending_grading_count > 0)
                                                <span class="badge bg-danger">
                                                    <i class="fas fa-exclamation-circle me-1"></i>
                                                    {{ $attempt->pending_grading_count }} بانتظار التصحيح
                                                </span>
                                            @else
                                                <span class="badge bg-success">
                                                    <i class="fas fa-check-circle me-1"></i>مُصحح
                                                </span>
                                            @endif
                                        </td>
                                        <td>
                                            <a href="{{ route('admin.question-module-grading.show', $attempt->id) }}"
                                               class="btn btn-sm btn-{{ $attempt->pending_grading_count > 0 ? 'danger' : 'primary' }}"
                                               title="تصحيح">
                                                <i class="fas fa-pen"></i>
                                            </a>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="8" class="text-center py-5">
                                            <div class="mb-3">
                                                <i class="fas fa-inbox fs-48 text-muted"></i>
                                            </div>
                                            <p class="text-muted fs-16">لا توجد محاولات للتصحيح</p>
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                </div>
                @if($attempts->hasPages())
                    <div class="card-footer">
                        {{ $attempts->links() }}
                    </div>
                @endif
            </div>

        </div>
    </div>
@stop

