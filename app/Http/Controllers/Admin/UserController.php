...
public function index(Request $request)
    {
        $data = $this->buildUsersIndexData($request);

        if ($response = $this->ajaxTableResponse(
            $request,
            $data,
            'admin.pages.users.partials.list',
            'admin.pages.users.partials.modals'
        )) {
            return $response;
        }
    ...
    /**
     * @return array{users: \Illuminate\Contracts\Pagination\LengthAwarePaginator, roles: \Illuminate\Support\Collection, sessions: \Illuminate\Support\Collection, stats: array<string, int>, onlineUserIds: array<int, mixed>}
     */
    private function buildUsersIndexData(Request $request): array
    {
        $roles = Role::all();
        ...
        $stats['filtered'] = $users->total();

        return compact('users', 'roles', 'sessions', 'stats', 'onlineUserIds');
    }
