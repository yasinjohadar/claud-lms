...
class BlogPostController extends Controller
{
    ...
    {
        $data = $this->buildPostsIndexData($request);

        if ($response = $this->ajaxTableResponse($request, $data, 'admin.blog.posts.partials.list')) {
            return $response;
        }
    ...
    /**
     * @return array{posts: \Illuminate\Contracts\Pagination\LengthAwarePaginator, categories: \Illuminate\Support\Collection, authors: \Illuminate\Support\Collection, stats: array<string, int|float>}
     */
    private function buildPostsIndexData(Request $request): array
    {
        ...
        $stats = [
            ...
            'views' => (int) BlogPost::sum('views_count'),
            'filtered' => $posts->total(),
        ];

        return compact('posts', 'categories', 'authors', 'stats');
    }
